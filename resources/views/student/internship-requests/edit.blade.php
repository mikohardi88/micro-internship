        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <h3 class="text-lg font-medium text-gray-700 mb-6">Edit Permintaan Magang</h3>

                    <form method="POST" action="{{ route('student.internship-requests.update', $internshipRequest) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-6">
                            ...
                            <select name="company_id" id="company_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                ...
                                @foreach($companies as $company)
                                    <option value="{{ $company->id }}" {{ old('company_id', $internshipRequest->company_id) == $company->id ? 'selected' : '' }}>
                                        {{ $company->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title', $internshipRequest->title) }}" 
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="description" id="description" rows="5" 
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" 
                                placeholder="Jelaskan tentang permintaan magang yang Anda ajukan..." required>{{ old('description', $internshipRequest->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <select name="duration_weeks" id="duration_weeks" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                @for($i = 1; $i <= 12; $i++)
                                    <option value="{{ $i }}" {{ old('duration_weeks', $internshipRequest->duration_weeks) == $i ? 'selected' : '' }}>
                                        {{ $i }} minggu
                                    </option>
                                @endfor
                            </select>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                                @php
                                    $selectedSkills = is_array($internshipRequest->skills_needed) ? $internshipRequest->skills_needed : json_decode($internshipRequest->skills_needed, true);
                                    $selectedSkills = $selectedSkills ?: [];
                                @endphp
                                @foreach($skills as $skill)
                                    <div class="flex items-center">
                                        <input type="checkbox" name="skills_needed[]" id="skill_{{ $skill->id }}" value="{{ $skill->id }}" 
                                            {{ in_array($skill->id, old('skills_needed', $selectedSkills)) ? 'checked' : '' }}
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="additional_notes" id="additional_notes" rows="3" 
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" 
                                placeholder="Tambahkan informasi tambahan yang relevan...">{{ old('additional_notes', $internshipRequest->additional_notes) }}</textarea>
