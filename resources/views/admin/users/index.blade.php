...
<head>
  ...
  <title>Manajemen Pengguna - Vinix Micro Internship</title>
...
<body class="bg-gray-50 font-sans">
  ...
  <aside id="sidebar" class="sidebar fixed left-0 top-0 h-full w-64 bg-white shadow-xl z-50">
    ...
    <nav class="p-4 space-y-2">
      ...
      <a href="{{ route('admin.users.index') }}" class="flex items-center space-x-3 p-3 rounded-lg bg-purple-100 text-purple-700 font-medium">
  ...
  <main class="lg:ml-64 p-6 lg:p-10">
    ...
    <div class="mb-8">
      <div class="flex justify-between items-center">
        ...
        <a href="{{ route('admin.users.create') }}" class="bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-lg font-medium">
      ...
      <form method="GET" action="{{ route('admin.users.index') }}">
        <div class="flex gap-4 flex-col md:flex-row">
          ...
          <div class="flex gap-2">
            ...
            @if(request('search'))
              <a href="{{ route('admin.users.index') }}" class="px-4 py-3 border border-gray-300 rounded-lg font-medium text-gray-700 hover:bg-gray-50">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex flex-wrap gap-1">
                    @foreach($user->roles as $role)
                      <span class="px-3 py-1 text-xs rounded-full bg-purple-100 text-purple-800 font-medium">{{ $role->name }}</span>
                    @endforeach
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <div class="flex justify-end space-x-2">
                    <a href="{{ route('admin.users.show', $user) }}" class="text-blue-600 hover:text-blue-900 p-2" title="Lihat Detail">
                      <i class="fas fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.users.edit', $user) }}" class="text-indigo-600 hover:text-indigo-900 p-2" title="Edit">
            ...
            @empty
              <tr>
                <td colspan="4" class="px-6 py-12 text-center text-sm text-gray-500">
                  ...
                  <p class="mt-1">Pengguna yang Anda cari tidak ditemukan</p>
                </td>
              </tr>
            @endforelse
          </tbody>
        ...
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
          <div class="flex justify-between items-center">
            <div class="text-sm text-gray-700">
              Menampilkan {{ $users->firstItem() }} hingga {{ $users->lastItem() }} dari {{ $users->total() }} total pengguna
            </div>
            <div class="pagination flex space-x-2">
              ...
              @else
                <a href="{{ $users->previousPageUrl() }}" class="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">
                  <i class="fas fa-chevron-left mr-1"></i>Sebelumnya
                </a>
              @endif

              @if($users->hasMorePages())
                <a href="{{ $users->nextPageUrl() }}" class="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">
