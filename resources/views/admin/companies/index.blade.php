...
<head>
  ...
  <title>Manajemen Perusahaan - Vinix Micro Internship</title>
...
<body class="bg-gray-50 font-sans">
  ...
  <aside id="sidebar" class="sidebar fixed left-0 top-0 h-full w-64 bg-white shadow-xl z-50">
    ...
    <nav class="p-4 space-y-2">
      ...
      <a href="{{ route('admin.companies.index') }}" class="flex items-center space-x-3 p-3 rounded-lg bg-purple-100 text-purple-700 font-medium">
  ...
  <main class="lg:ml-64 p-6 lg:p-10">
    ...
    <div class="mb-8">
      <div class="flex justify-between items-center">
        ...
        <a href="{{ route('admin.companies.create') }}" class="bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-lg font-medium">
        ...
        <div class="flex gap-4 flex-col md:flex-row">
          <div class="flex-1">
            <input name="search" type="text" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                   placeholder="Cari perusahaan berdasarkan nama atau industri..." value="{{ request('search') }}" />
          </div>
          <div class="flex gap-2">
            ...
            @if(request('search'))
              <a href="{{ route('admin.companies.index') }}" class="px-4 py-3 border border-gray-300 rounded-lg font-medium text-gray-700 hover:bg-gray-50">
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Pemilik</th>
              <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Industri</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            @forelse($companies as $company)
              <tr class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm text-gray-900">{{ $company->user->name ?? 'N/A' }}</div>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <div class="flex justify-end space-x-2">
                    <a href="{{ route('admin.companies.show', $company) }}" class="text-blue-600 hover:text-blue-900 p-2" title="Lihat Detail">
                      <i class="fas fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.companies.edit', $company) }}" class="text-indigo-600 hover:text-indigo-900 p-2" title="Edit">
                      <i class="fas fa-edit"></i>
                    </a>
                    <form action="{{ route('admin.companies.destroy', $company) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus perusahaan ini?')">
                      @csrf
                      @method('DELETE')
            ...
            @empty
              <tr>
                <td colspan="4" class="px-6 py-12 text-center text-sm text-gray-500">
                  ...
                  <p class="font-medium">Tidak ada perusahaan ditemukan</p>
                  <p class="mt-1">Perusahaan yang Anda cari tidak ditemukan</p>
                </td>
              </tr>
            @endforelse
          </tbody>
        ...
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
          <div class="flex justify-between items-center">
            <div class="text-sm text-gray-700">
              Menampilkan {{ $companies->firstItem() }} hingga {{ $companies->lastItem() }} dari {{ $companies->total() }} total perusahaan
            </div>
            <div class="pagination flex space-x-2">
              ...
              @else
                <a href="{{ $companies->previousPageUrl() }}" class="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">
                  <i class="fas fa-chevron-left mr-1"></i>Sebelumnya
                </a>
              @endif

              @if($companies->hasMorePages())
                <a href="{{ $companies->nextPageUrl() }}" class="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">
