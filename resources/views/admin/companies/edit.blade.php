...
<head>
  ...
  <title>Edit Perusahaan - Vinix Micro Internship</title>
  ...
  <main class="lg:ml-64 p-6 lg:p-10">
    ...
    <div class="mb-8">
      <h1 class="text-3xl font-bold text-gray-800">
        <a href="{{ route('admin.companies.index') }}" class="text-purple-600 hover:text-purple-800">Perusahaan</a> / Edit Perusahaan
      </h1>
    </div>
    ...
    <div class="bg-white p-8 rounded-xl shadow-sm border">
      <form method="POST" action="{{ route('admin.companies.update', $company) }}">
        ...
        <div class="mb-6">
          ...
          <select id="user_id" name="user_id" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" required>
            @foreach($users as $user)
              <option value="{{ $user->id }}" @selected(old('user_id', $company->user_id) == $user->id)>{{ $user->name }} ({{ $user->email }})</option>
            @endforeach
          </select>
          @error('user_id')
        ...
        <div class="mb-6">
          ...
          <input id="name" name="name" type="text" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" value="{{ old('name', $company->name) }}" required />
          @error('name')
            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
          @enderror
        </div>
        ...
        <div class="mb-6">
          ...
          <textarea id="description" name="description" rows="4" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">{{ old('description', $company->description) }}</textarea>
          @error('description')
            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
          @enderror
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div>
            ...
            <input id="industry" name="industry" type="text" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" value="{{ old('industry', $company->industry) }}" />
            @error('industry')
              <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            @enderror
          </div>
          <div>
            ...
            <input id="size" name="size" type="text" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" value="{{ old('size', $company->size) }}" />
            @error('size')
              <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            @enderror
          </div>
        ...
        <div class="mb-8">
          ...
          <input id="website" name="website" type="url" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" value="{{ old('website', $company->website) }}" />
          @error('website')
            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
          @enderror
        </div>
        ...
        <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-200">
          ...
          <button type="submit" class="px-6 py-3 bg-purple-600 text-white rounded-lg font-medium hover:bg-purple-700">
            <i class="fas fa-save mr-2"></i>Update Perusahaan
          </button>
        </div>
      </form>
    </div>
