...
<head>
  ...
  <title>Edit Kemampuan - Vinix Micro Internship</title>
  ...
  <main class="lg:ml-64 p-6 lg:p-10">
    ...
    <div class="mb-8">
      <h1 class="text-3xl font-bold text-gray-800">
        <a href="{{ route('admin.skills.index') }}" class="text-purple-600 hover:text-purple-800">Kemampuan</a> / Edit Kemampuan
      </h1>
    </div>
    ...
    <div class="bg-white p-8 rounded-xl shadow-sm border">
      <form method="POST" action="{{ route('admin.skills.update', $skill) }}">
        ...
        <div class="mb-6">
          ...
          <input id="name" name="name" type="text" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" value="{{ old('name', $skill->name) }}" required />
          @error('name')
        ...
        <div class="mb-8">
          ...
          <textarea id="description" name="description" rows="4" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">{{ old('description', $skill->description) }}</textarea>
          @error('description')
            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
          @enderror
        </div>
        ...
        <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-200">
          <a href="{{ route('admin.skills.index') }}" class="px-6 py-3 border border-gray-300 rounded-lg font-medium text-gray-700 hover:bg-gray-50">
            <i class="fas fa-arrow-left mr-2"></i>Batal
          </a>
          <button type="submit" class="px-6 py-3 bg-purple-600 text-white rounded-lg font-medium hover:bg-purple-700">
            <i class="fas fa-save mr-2"></i>Update Kemampuan
          </button>
        </div>
      </form>
    </div>
