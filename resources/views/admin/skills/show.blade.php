...
<head>
  ...
  <title>Detail Kemampuan - Vinix Micro Internship</title>
    ...
    <div class="mb-8">
      <div class="flex justify-between items-center">
        <h1 class="text-3xl font-bold text-gray-800">
          <a href="{{ route('admin.skills.index') }}" class="text-purple-600 hover:text-purple-800">Kemampuan</a> / {{ $skill->name }}
        </h1>
        <div class="flex gap-3">
          <a href="{{ route('admin.skills.edit', $skill) }}" class="px-6 py-3 bg-purple-600 text-white rounded-lg font-medium hover:bg-purple-700">
            <i class="fas fa-edit mr-2"></i>Edit
          </a>
          <form action="{{ route('admin.skills.destroy', $skill) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kemampuan ini?')">
            @csrf
            @method('DELETE')
    ...
    <div class="bg-white p-8 rounded-xl shadow-sm border">
      ...
      @if($skill->description)
        <div class="mb-8 pt-6 border-t border-gray-200">
          ...
          <p class="text-gray-900 whitespace-pre-wrap">{{ $skill->description }}</p>
        </div>
      @endif

      <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-6 border-t border-gray-200">
        <div>
          <h3 class="text-sm font-medium text-gray-500 mb-3">Pengguna dengan kemampuan ini</h3>
          <p class="text-gray-900">{{ $skill->users->count() }} pengguna</p>
        </div>

        <div>
          <h3 class="text-sm font-medium text-gray-500 mb-3">Proyek yang membutuhkan kemampuan ini</h3>
          <p class="text-gray-900">{{ $skill->projects->count() }} proyek</p>
        </div>
      </div>
    </div>
  </main>
